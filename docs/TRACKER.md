...
| M7-6 | Pre-launch security checklist | M | doing | `docs/SECURITY.md` §7 walked item-by-item against live production (2026-09-25), read-only. Confirmed good: HTTPS/HSTS/CSP, app dir + `.env`/`.git` unreachable, DB privilege limitation accepted. **Found real gaps, not yet closed:** `preflight.php` isn't run anywhere in `deploy.yml`; `storage/logs` and `rate_limits` both grow forever — no cron/scheduled-task mechanism exists on this project at all, so `RateLimiter::prune()` (now tested but never called) has nowhere to run from; error alerting doesn't exist (no mail/webhook/tracker integration despite `MAIL_HOST` being provisioned and unused). Also closed a real test-coverage gap: `RateLimiter` had zero tests despite CLAUDE.md rule 9 — added `tests/Feature/RateLimiterTest.php`. Remaining items need either owner decisions (how to schedule tasks on Hostinger — cron vs GitHub Actions; what error alerting should even notify) or on-host/DB access this pass didn't have |
